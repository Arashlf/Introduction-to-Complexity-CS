...
to-report new [xys]  ;; calculate new generation using counter table
  ...
  foreach xys [ ?1 -> table:put counters ?1 0 ] ;; so we can tell new from old
  foreach xys
    [ ?1 -> set x first ?1 set y last ?1 ;; cache local values
     foreach [[-1  1] [0  1] [1  1]  ;; count up the eight neighbors
              [-1  0]        [1  0]  ;; of each 'live' cell
              [-1 -1] [0 -1] [1 -1]]
         [ ??1 -> set c list (x + first ??1) (y + last ??1)  ;; c for "cell", here
  ...
  foreach table:to-list counters  ;; filter out 'dead' cells
     [ ?1 -> set c last ?1  ;; reuse "c" for "count"
      if c = 2 or c = 3 or c = 3.1 [set xys lput (first ?1) xys] ]
  report xys
end

to depict [xys] ;; assuming the patches have been set up
  no-display ask turtles [hide-turtle]  foreach visible offset xys
    [ ?1 -> ask patch (first ?1) (last ?1) [ask turtles-here [show-turtle]] ] display
end
...
to preview [xys]  recolor-patches
  foreach visible xys [ ?1 -> ask patch (first ?1) (last ?1) [set pcolor gray + 2] ]
end
...
to-report visible [xys] report filter
  [ ?1 -> min-pxcor <= first ?1 and first ?1 <= max-pxcor and
   min-pycor <= last  ?1 and last  ?1 <= max-pycor ] xys
end
...
to-report translated-by [d-x d-y xys]
  report map [ ?1 -> list (d-x + first ?1) (d-y + last ?1) ] xys
end

to-report centered [xys]  ;; center around 'average' location
  let sums reduce [ [?1 ?2] -> list (first ?1 + first ?2) (last ?1 + last ?2) ] xys
  let d-x round (first sums / length xys)
  let d-y round (last  sums / length xys)
  report translated-by (0 - d-x) (0 - d-y) xys
end

to-report x-flipped [xys] report map [ ?1 -> list (0 - first ?1) (last ?1) ] xys end
to-report y-flipped [xys] report map [ ?1 -> list (first ?1) (0 - last ?1) ] xys end
...
If you use this model, please cite it as: "GameOfLife" model, Complexity Explorer project, http://complexityexplorer.org
